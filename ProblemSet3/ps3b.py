...
class Patient(object):
    ...
    def update(self):
        ...
        # Determine if each particule survives and update the list accordingly
        for i in viruses_bis:
            if i.doesClear():
                viruses.remove(i)
        ...
        # Determine for each virus particule if it reproduces or not
        viruses_bis = viruses[:]
        for i in viruses_bis:
            try:
                viruses.append(i.reproduce(PopDensity)) 
...
def simulationWithoutDrug(numViruses, maxPop, maxBirthProb, clearProb,
    ...
    for i in range(numTrials):
        ...
        #Patient_list.append(Patient(viruses, maxPop))
        for t in range(300):
            Result[t] = Result[t] + P.update()/numTrials
...
class ResistantVirus(SimpleVirus):
    ...
    def __init__(self, maxBirthProb, clearProb, resistances, mutProb):
        ...
        super().__init__(self,maxBirthProb, clearProb)
    ...
    def reproduce(self, popDensity, activeDrugs):
        ...
        for i in activeDrugs:
            if not resistances[i]:
                resistant = False
        if not resistant:
            raise NoChildException
        else:
            if random.random()<self.maxBirthProb * (1 - popDensity):
                new_resistances = resistances.copy()
                for k in new_resistances:
                    if random.random()<self.mutProb:
                        new_resistances[k] = not(new_resistances[k])
