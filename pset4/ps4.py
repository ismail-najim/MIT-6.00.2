...
def generate_models(x, y, degs):
    ...
    solution = []
    for i in degs:
        solution.append(np.polyfit(x,y,i))
    return solution
...
def r_squared(y, estimated):
    ...
    """
    y_array = np.array(y)
    e_array = np.array(estimated)
    mean = np.average(y_array)
    v1 = (y_array - e_array)**2
    v2 = (y_array - mean)**2
    R_2 = 1-v1.sum()/v2.sum()
    return R_2
...
def evaluate_models_on_training(x, y, models):
    ...
    for m in models:
        i += 1
        y_est = pylab.polyval(m, x)
        r_2 = r_squared(y, y_est)
        pylab.figure()
        pylab.plot(x, y, 'bo', label = 'measured points')
        pylab.plot(x, y_est, 'r--', label = 'model'+str(i) )
        pylab.title('model '+str(m)+' r^2 '+str(r_2))
